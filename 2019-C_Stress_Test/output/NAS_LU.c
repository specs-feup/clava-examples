...
void clava_call_graph() {
   FILE *log_file_368 = fopen("/home/specs/jbispo/repos/clava-examples/2019-C_Stress_Test/output/NAS_LU.dot", "w+");
   if (log_file_368 == NULL)
   {
       printf("Error opening file /home/specs/jbispo/repos/clava-examples/2019-C_Stress_Test/output/NAS_LU.dot\n");
       exit(1);
   } 
   fprintf(log_file_368, "digraph dynamic_call_graph {\n\n");
   if(clava_dcg_global[0] != 0) {
      fprintf(log_file_368, "	main -> read_input [label=\"%d\"];\n", clava_dcg_global[0]);
   }
   if(clava_dcg_global[1] != 0) {
      fprintf(log_file_368, "	main -> domain [label=\"%d\"];\n", clava_dcg_global[1]);
   }
   if(clava_dcg_global[2] != 0) {
      fprintf(log_file_368, "	main -> setcoeff [label=\"%d\"];\n", clava_dcg_global[2]);
   }
   if(clava_dcg_global[3] != 0) {
      fprintf(log_file_368, "	main -> setbv [label=\"%d\"];\n", clava_dcg_global[3]);
   }
   if(clava_dcg_global[4] != 0) {
      fprintf(log_file_368, "	main -> setiv [label=\"%d\"];\n", clava_dcg_global[4]);
   }
   if(clava_dcg_global[5] != 0) {
      fprintf(log_file_368, "	main -> erhs [label=\"%d\"];\n", clava_dcg_global[5]);
   }
   if(clava_dcg_global[6] != 0) {
      fprintf(log_file_368, "	main -> ssor [label=\"%d\"];\n", clava_dcg_global[6]);
   }
   if(clava_dcg_global[7] != 0) {
      fprintf(log_file_368, "	main -> error [label=\"%d\"];\n", clava_dcg_global[7]);
   }
   if(clava_dcg_global[8] != 0) {
      fprintf(log_file_368, "	main -> pintgr [label=\"%d\"];\n", clava_dcg_global[8]);
   }
   if(clava_dcg_global[9] != 0) {
      fprintf(log_file_368, "	main -> verify [label=\"%d\"];\n", clava_dcg_global[9]);
   }
   if(clava_dcg_global[10] != 0) {
      fprintf(log_file_368, "	main -> pow [label=\"%d\"];\n", clava_dcg_global[10]);
   }
   if(clava_dcg_global[11] != 0) {
      fprintf(log_file_368, "	main -> print_results [label=\"%d\"];\n", clava_dcg_global[11]);
   }
   if(clava_dcg_global[12] != 0) {
      fprintf(log_file_368, "	domain -> printf [label=\"%d\"];\n", clava_dcg_global[12]);
   }
   if(clava_dcg_global[13] != 0) {
      fprintf(log_file_368, "	domain -> exit [label=\"%d\"];\n", clava_dcg_global[13]);
   }
   if(clava_dcg_global[14] != 0) {
      fprintf(log_file_368, "	error -> exact [label=\"%d\"];\n", clava_dcg_global[14]);
   }
   if(clava_dcg_global[15] != 0) {
      fprintf(log_file_368, "	error -> sqrt [label=\"%d\"];\n", clava_dcg_global[15]);
   }
   if(clava_dcg_global[16] != 0) {
      fprintf(log_file_368, "	l2norm -> sqrt [label=\"%d\"];\n", clava_dcg_global[16]);
   }
   if(clava_dcg_global[17] != 0) {
      fprintf(log_file_368, "	read_input -> printf [label=\"%d\"];\n", clava_dcg_global[17]);
   }
   if(clava_dcg_global[18] != 0) {
      fprintf(log_file_368, "	read_input -> fopen [label=\"%d\"];\n", clava_dcg_global[18]);
   }
   if(clava_dcg_global[19] != 0) {
      fprintf(log_file_368, "	read_input -> fgetc [label=\"%d\"];\n", clava_dcg_global[19]);
   }
   if(clava_dcg_global[20] != 0) {
      fprintf(log_file_368, "	read_input -> fscanf [label=\"%d\"];\n", clava_dcg_global[20]);
   }
   if(clava_dcg_global[21] != 0) {
      fprintf(log_file_368, "	read_input -> fclose [label=\"%d\"];\n", clava_dcg_global[21]);
   }
   if(clava_dcg_global[22] != 0) {
      fprintf(log_file_368, "	read_input -> exit [label=\"%d\"];\n", clava_dcg_global[22]);
   }
   if(clava_dcg_global[23] != 0) {
      fprintf(log_file_368, "	setbv -> exact [label=\"%d\"];\n", clava_dcg_global[23]);
   }
   if(clava_dcg_global[24] != 0) {
      fprintf(log_file_368, "	setiv -> exact [label=\"%d\"];\n", clava_dcg_global[24]);
   }
   if(clava_dcg_global[25] != 0) {
      fprintf(log_file_368, "	ssor -> timer_clear [label=\"%d\"];\n", clava_dcg_global[25]);
   }
   if(clava_dcg_global[26] != 0) {
      fprintf(log_file_368, "	ssor -> rhs [label=\"%d\"];\n", clava_dcg_global[26]);
   }
   if(clava_dcg_global[27] != 0) {
      fprintf(log_file_368, "	ssor -> l2norm [label=\"%d\"];\n", clava_dcg_global[27]);
   }
   if(clava_dcg_global[28] != 0) {
      fprintf(log_file_368, "	ssor -> timer_start [label=\"%d\"];\n", clava_dcg_global[28]);
   }
   if(clava_dcg_global[29] != 0) {
      fprintf(log_file_368, "	ssor -> printf [label=\"%d\"];\n", clava_dcg_global[29]);
   }
   if(clava_dcg_global[30] != 0) {
      fprintf(log_file_368, "	ssor -> jacld [label=\"%d\"];\n", clava_dcg_global[30]);
   }
   if(clava_dcg_global[31] != 0) {
      fprintf(log_file_368, "	ssor -> blts [label=\"%d\"];\n", clava_dcg_global[31]);
   }
   if(clava_dcg_global[32] != 0) {
      fprintf(log_file_368, "	ssor -> jacu [label=\"%d\"];\n", clava_dcg_global[32]);
   }
   if(clava_dcg_global[33] != 0) {
      fprintf(log_file_368, "	ssor -> buts [label=\"%d\"];\n", clava_dcg_global[33]);
   }
   if(clava_dcg_global[34] != 0) {
      fprintf(log_file_368, "	ssor -> timer_stop [label=\"%d\"];\n", clava_dcg_global[34]);
   }
   if(clava_dcg_global[35] != 0) {
      fprintf(log_file_368, "	ssor -> timer_read [label=\"%d\"];\n", clava_dcg_global[35]);
   }
   if(clava_dcg_global[36] != 0) {
      fprintf(log_file_368, "	verify -> fabs [label=\"%d\"];\n", clava_dcg_global[36]);
   }
   if(clava_dcg_global[37] != 0) {
      fprintf(log_file_368, "	verify -> printf [label=\"%d\"];\n", clava_dcg_global[37]);
   }
   if(clava_dcg_global[38] != 0) {
      fprintf(log_file_368, "	print_results -> printf [label=\"%d\"];\n", clava_dcg_global[38]);
   }
   if(clava_dcg_global[39] != 0) {
      fprintf(log_file_368, "	print_results -> sprintf [label=\"%d\"];\n", clava_dcg_global[39]);
   }
   if(clava_dcg_global[40] != 0) {
      fprintf(log_file_368, "	print_results -> pow [label=\"%d\"];\n", clava_dcg_global[40]);
   }
   if(clava_dcg_global[41] != 0) {
      fprintf(log_file_368, "	wtime -> gettimeofday [label=\"%d\"];\n", clava_dcg_global[41]);
   }
   if(clava_dcg_global[42] != 0) {
      fprintf(log_file_368, "	elapsed_time -> wtime [label=\"%d\"];\n", clava_dcg_global[42]);
   }
   if(clava_dcg_global[43] != 0) {
      fprintf(log_file_368, "	timer_start -> elapsed_time [label=\"%d\"];\n", clava_dcg_global[43]);
   }
   if(clava_dcg_global[44] != 0) {
      fprintf(log_file_368, "	timer_stop -> elapsed_time [label=\"%d\"];\n", clava_dcg_global[44]);
   }
   fprintf(log_file_368, "}\n");
   fclose(log_file_368);
}
